...
def apply_image_effect(image_path, effect_type):
    ...
    try:
        ...
        processed_image_path = os.path.join(
            temp_dir, f"processed_wallpaper_{effect_type.lower()}.jpg"
        )
        ...
        show_error_dialog(
            f"Cannot open or identify image file: {image_path}. It might be corrupted or an unsupported format."
        )
        return image_path
    except IOError as e:
        logging.error(
            f"File I/O error while applying image effect {effect_type} to {image_path}: {e}"
        )
        show_error_dialog(
            f"Could not save processed image with {effect_type} effect. Check disk space or permissions."
        )
        return image_path
        ...
        show_error_dialog(
            "An unexpected critical error occurred while applying image effect. Please check the logs for details."
        )
        return image_path
